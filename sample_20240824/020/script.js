...
// = 020 ======================================================================
// ホワイトノイズは単純に乱数をピクセル単位で求めることで、まるで砂嵐のような外
// 見になりますが、これを応用して組み合わせてやることで、より複雑な模様を描画す
// ることもできるようになります。
// ここでは、バリューノイズと呼ばれるノイズ生成のテクニックを解説します。
// 基本的な実装はすべてポストエフェクトのシェーダ（render.frag）内で実装してある
// のですが、細かく１つ１つの処理を追いかけていくのはちょっと大変かもしれません。
// ポイントは、ものすごく大量の乱数生成を行い結果を補間・合成することで、より複
// 雑な模様を生成しているという点でしょう。
// 難易度が高いジャンルになりますので、まずはその考え方や、あるいは活用の仕方な
// どから考えてみるのがよいでしょう。
// ============================================================================
...
class App {
  ...
  setupPane() {
    ...
    // グレイスケール化するかどうか
    pane.addBinding(parameter, 'grayScale')
    .on('change', (v) => {
  ...
  setupLocation() {
    ...
    this.renderUniLocation = {
      ...
      grayScale: gl.getUniformLocation(this.renderProgram, 'grayScale'),     // グレイスケール化するかどうか @@@
  ...
  render() {
    ...
    // - 最終シーンのレンダリング ---------------------------------------------
    {
      ...
      gl.uniform1i(this.renderUniLocation.grayScale, this.isGrayScale); // グレイスケール化するかどうか @@@
